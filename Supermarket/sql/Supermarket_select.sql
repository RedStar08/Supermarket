...
-- 今日销售额
SELECT sum(salesSum) FROM `tb_sale` where salesDate ='2019-11-15';
-- 总销售额
...
-- 采购总价计算
select  tb_sales.salesID,sum(salesNum)
from tb_sales,tb_sale
where tb_sales.salesID = tb_sale.salesID
group by tb_sales.salesID
...
-- ----------------------------
-- 1、增：
INSERT INTO `tb_outstocks` VALUES ('{$outstockID}', '{$storeID}', '{$goodsID}', '{$supplierID}', 
'{$produceID}', '{$outstockNum}', '{$note}');
INSERT INTO `tb_instocks` VALUES ('202001010002', '400002', '000024', '300008', '20200101', '200', '元旦入库');
-- 2、删：
Delete from tb_outstocks where goodsID = '{$goodsID}' and supplierID = '{$supplierID}' 
and storeID = '{$storeID}' and outstockID = '{$outstockID}' and produceID = '{$produceID}' ;

-- 3、改：
update tb_outstocks set goodsID = '{$goodsID}' and supplierID = '{$supplierID}' and storeID = '{$storeID}' and outstockID = '{$outstockID}' 
and produceID = '{$produceID}' and outstockNum = '{$outstockNum}', note = '{$note}' 
where goodsID = '{$goodsID}' and supplierID = '{$supplierID}' 
and storeID = '{$storeID}' and outstockID = '{$outstockID}' and produceID = '{$produceID}' ;


-- 4、查：
select * from tb_outstocks;

-- 以下针对下拉框添加
-- 确定商品ID时
select  distinct supplierID from tb_produce where goodsID = '{$goodsID}';
select  distinct produceID from tb_produce where goodsID = '{$goodsID}';

-- 确定供应商ID时
select  distinct goodsID from tb_produce where supplierID = '{$supplierID}';
select  distinct produceID from tb_produce where supplierID = '{$supplierID}';

-- 确定生产批号时
select  distinct goodsID from tb_produce where produceID='{$produceID}';
select  distinct supplierID from tb_produce where produceID='{$produceID}';

-- 确定两个属性值时
select  distinct goodsID from tb_produce where produceID='{$produceID}' and supplierID = '{$supplierID}';
select  distinct supplierID from tb_produce where goodsID = '{$goodsID}' and produceID='{$produceID}';
select  distinct produceID from tb_produce where goodsID = '{$goodsID}' and supplierID = '{$supplierID}' ;
...
create view outstocks as select  tb_outstocks.outstockID,CONCAT_WS('',tb_outstocks.goodsID,tb_goods.goodsName),CONCAT_WS('',tb_outstocks.supplierID,tb_supplier.supplierName),
	tb_outstocks.produceID,CONCAT_WS('',tb_outstocks.storeID,tb_store.storeName),tb_outstocks.outstockNum,tb_outstocks.note
...
-- create view outstock as 
-- 	select  tb_outstock.outstockID,tb_outstock.outDate,sum(outstockNum),tb_outstock.note
-- 	from tb_outstock,tb_outstocks
-- 	where tb_outstock.outstockID = tb_outstocks.outstockID 
-- 	group by tb_outstock.outstockID
...
-- ----------------------------
-- 1、增：
INSERT INTO `tb_instocks` VALUES ('{$instockID}', '{$storeID}', '{$goodsID}', '{$supplierID}', 
'{$produceID}', '{$instockNum}', '{$note}');

-- 2、删：
Delete from tb_instocks where goodsID = '{$goodsID}' and supplierID = '{$supplierID}' 
and storeID = '{$storeID}' and instockID = '{$instockID}' and produceID = '{$produceID}' ;

-- 3、改：
update tb_instocks set goodsID = '{$goodsID}' and supplierID = '{$supplierID}' and storeID = '{$storeID}' and instockID = '{$instockID}' 
and produceID = '{$produceID}' and instockNum = '{$instockNum}', note = '{$note}' 
where goodsID = '{$goodsID}' and supplierID = '{$supplierID}' 
and storeID = '{$storeID}' and instockID = '{$instockID}' and produceID = '{$produceID}' ;


-- 4、查：
select * from tb_instocks;
...
-- select  distinct goodsID from tb_produce where buysID='{$buysID}';
-- select  distinct supplierID from tb_produce where buysID='{$buysID}';
-- select  distinct produceID from tb_produce where buysID='{$buysID}';
-- select  distinct storeID from tb_produce where buysID='{$buysID}';

-- select  distinct goodsID from tb_produce where buysID='{$instockID}';
-- select  distinct supplierID from tb_produce where buysID='{$instockID}';
-- select  distinct produceID from tb_produce where buysID='{$instockID}';
-- select  distinct storeID from tb_produce where buysID='{$instockID}';
...
-- 查询视图
select  distinct goodsID,goodsName,supplierID,supplierName,storeID,storeName from instocks 
where goodsID = '{$goodsID}' and storeID = '{$storeID}' and supplierID = '{$supplierID}';
...
create view instocks as select  tb_instocks.instockID,CONCAT_WS('',tb_instocks.goodsID,tb_goods.goodsName),CONCAT_WS('',tb_instocks.supplierID,tb_supplier.supplierName),
	tb_instocks.produceID,CONCAT_WS('',tb_instocks.storeID,tb_store.storeName),tb_instocks.instockNum,tb_instocks.note
...
-- create view instocks as select tb_goods.goodsID,goodsName,goodsType,goodsPrice,goodsSpecs,goodsSave,supplierName,produceID,produceDate,produceArea,tb_goodinfo.note 
-- from tb_goods,tb_goodinfo,tb_supplier 
-- where tb_goods.goodsID = tb_goodinfo.goodsID and tb_supplier.supplierID = tb_goodinfo.supplierID


-- DROP view IF EXISTS `instocks`;
-- create view instocks as select  tb_instocks.goodsID,tb_goods.goodsName,tb_instocks.supplierID,tb_supplier.supplierName,
-- 	tb_instocks.produceID,tb_instocks.storeID,tb_store.storeName,tb_instocks.instockNum,tb_instocks.note
-- from tb_goods,tb_store,tb_supplier,tb_instocks
-- where tb_goods.goodsID = tb_instocks.goodsID and tb_supplier.supplierID = tb_instocks.supplierID and tb_instocks.storeID = tb_store.storeID

-- 入库与生产表的级联(下拉框)
DROP view IF EXISTS `instocks_produce`;
create view instocks_produce as select tb_produce.goodsID,goodsName,tb_produce.supplierID,supplierName,produceID
from tb_goods,tb_supplier,tb_produce 
where tb_produce.goodsID = tb_goods.goodsID 
and tb_produce.supplierID = tb_supplier.supplierID
...
create view buy as 
	select  tb_buy.buysID,tb_buy.buyDate,sum(buyNum),tb_buy.note
	from tb_buy,tb_buys
	where tb_buy.buysID = tb_buys.buysID 
	group by tb_buy.buysID 
