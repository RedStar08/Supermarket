<?php
    ...
    if(isset($_GET['instockID'])){
        //查询详情
        $sql = "select * from instocks where instockID = '{$_GET['instockID']}'";
        $query = mysqli_query($con,$sql);
        $rows = mysqli_fetch_all($query,MYSQLI_ASSOC);
        
        //商品数
        $sql = "select count(distinct goodsID) from tb_instocks where instockID='{$_GET['instockID']}';";
        $query = mysqli_query($con, $sql);
        $nums = mysqli_fetch_array($query)[0];
        
        //查看详情失败
        if($rows == NULL){
            echo "<script>alert('查看详情失败!请检查数据库');window.location.href='instock-list.php';</script>";  
        }else{
            //传给js
            $new_data = array();
            foreach ($rows as $row) {
                ...
                $tem_data['instockNum'] = $row['instockNum'];
  ...
  <body class="layui-layout-body">
    ...
    <div class="layui-fluid">
      <fieldset class="layui-elem-field layui-field-title" style="text-align: center;">
          <legend>入库单号：<?php echo $_GET['instockID']?></legend>
          <legend>日期：<?php echo $_GET['inDate']?></legend>
      </fieldset>
      <!-- 自动渲染表格 -->
...
<script>
  ...
  layui.use(['table'], function(){
    ...
    // 表格数据接口 
    var instock_detail =  <?php echo json_encode($new_data);?>;
    //表格设置
    table.render({
      elem: '#instock-list'
      ,totalRow: true
      ...
      ,cols: [[ //标题栏
        ...
        ,{field: 'instockNum', title: '入库数量', sort: true, totalRow: true}
        ,{field: 'note', title: '备注'}
      ]]
      ,data: instock_detail
